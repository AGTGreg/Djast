...
DIST_DIR = Path(__file__).resolve().parent.parent / "frontend" / "build_output"
...
async def test_admin_spa_favicon(client):
    """Static files in build_output root are served directly."""
...
async def test_admin_spa_js_asset(client):
    ...
    assert js_files, "No JS bundle found in build_output/assets/"
...
async def test_admin_spa_css_asset(client):
    ...
    assert css_files, "No CSS bundle found in build_output/assets/"
...
async def test_path_traversal_blocked(client):
    """Percent-encoded path traversal must not leak files outside build_output.
