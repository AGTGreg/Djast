...
def _cast_pk(pk_col: Any, value: str | int) -> Any | None:
    """Cast a PK value to the column's Python type.

    Returns None if the value cannot be converted (e.g. "abc" for an
    integer PK), letting callers treat it as "not found".
    """
    try:
        return pk_col.property.columns[0].type.python_type(value)
...
async def get_record(
    ...
    pk_value = _cast_pk(pk_col, record_id)
    if pk_value is None:
        return None
...
async def bulk_delete_records(
    ...
    model = entry.model_class
    pk_col = _get_pk_col(entry)
    cast_ids = [v for i in ids if (v := _cast_pk(pk_col, i)) is not None]
    if not cast_ids:
        return 0
    stmt = sa_delete(model).where(pk_col.in_(cast_ids))
