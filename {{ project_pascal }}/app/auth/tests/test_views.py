...
from auth.tests.helpers import (
    ...
    password_of_length as _password_of_length,
...
async def csrf_client():
    """Standalone FastAPI app with one route guarded by ``csrf_protect``.

    ``csrf_protect`` is a pure function of the request's cookies and headers —
    it has no coupling to ``main.app``. Testing it against a throwaway app
    avoids mutating the shared ``main.app.routes`` list and keeps these tests
    independent of auth-mode parametrization.
    """
...
async def test_signup_success(auth_client):
    client, mode, _db_session = auth_client
...
async def test_refresh_missing_cookie_returns_400(auth_client):
    client, _mode, _db_session = auth_client
...
@pytest.mark.parametrize(
    "endpoint",
    PROTECTED_ENDPOINTS,
    ids=lambda e: f"{e.method}_{e.path.strip('/').replace('/', '_')}",
)
@pytest.mark.asyncio
async def test_protected_endpoint_rejects_missing_token(auth_client, endpoint):
    """Every protected endpoint must return 401 when called without auth.

    Guards against wiring regressions: if someone accidentally removes
    ``Depends(get_current_user)`` from an endpoint, it silently becomes
    public. This test fails the moment that happens.
    """
    client, _mode, _db_session = auth_client
    request_kwargs = {}
    ...
    resp = await client.request(
        endpoint.method,
        f"{_auth_prefix()}{endpoint.path}",
        **request_kwargs,
    )

    assert resp.status_code == 401, (
        f"{endpoint.method} {endpoint.path}: expected 401, got "
...
@pytest.mark.asyncio
async def test_csrf_protect_rejects_missing_header(csrf_client):
    """csrf_protect returns 403 when the X-CSRF-Token header is missing."""
    from djast.utils.csrf import generate_csrf_token

    csrf_token = generate_csrf_token()

    resp = await csrf_client.post(
        "/protected",
        headers={"Cookie": f"{settings.CSRF_COOKIE_NAME}={csrf_token}"},
    )
    assert resp.status_code == 403
...
@pytest.mark.asyncio
async def test_csrf_protect_rejects_wrong_token(csrf_client):
    """csrf_protect returns 403 when the header doesn't match the cookie."""
    from djast.utils.csrf import generate_csrf_token
...
@pytest.mark.asyncio
async def test_csrf_protect_succeeds_with_correct_token(csrf_client):
    """csrf_protect passes through when header matches cookie."""
    from djast.utils.csrf import generate_csrf_token
