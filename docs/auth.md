...
This document covers user models, authentication endpoints, token lifecycle, email verification, password reset, OAuth2, and security configuration.
...
- Django-compatible password hashing (PBKDF2-SHA256) — works with existing Django databases.
- Optional OAuth2 social login (Google, GitHub) — disabled by default.
...
- Security layers: token blacklisting via Redis, brute-force protection, rate limiting.

---

## Choosing a User Model

Set `AUTH_USER_MODEL_TYPE` in your environment (or `dev.env`):

| Value | User model | Login field | Extra fields |
|-------|-----------|------------|--------------|
| `"django"` (default) | `AbstractDjangoUser` | `username` | `email`, `first_name`, `last_name` |
| `"email"` | `AbstractEmailUser` | `email` | (none) |

Both models share a common base (`AbstractBaseUser`) that provides `password`, `is_active`, `is_staff`, `is_superuser`, `date_joined`, and `last_login`.

The Django model is fully compatible with Django's `auth_user` table. If you have an existing Django database with users, Djast can authenticate against it — the password hashing format (`pbkdf2_sha256`) is identical.

Switch this **before running your first migration** — changing it later requires a migration to alter the `auth_user` table.
...
Controlled by the `EMAIL_VERIFICATION` setting:

| Value | Behavior |
|-------|----------|
| `"none"` (default) | No verification emails sent. Users can log in immediately. |
| `"optional"` | Verification email sent on signup, but login is allowed without it. |
| `"mandatory"` | Verification email sent on signup. Users **cannot log in** until verified (login returns 403). |
...
Returns 400 if already verified or no email address. Returns 429 if sent within the last 5 minutes (configurable via `EMAIL_COOLDOWN_SECONDS`).

### Mandatory verification and login

When `EMAIL_VERIFICATION=mandatory`, the login endpoint returns 403:
```json
{"detail": "Email verification required."}
```

For DjangoUser accounts without an email address, the verification gate is bypassed — there's nothing to verify.

---

## Password Reset

The password reset flow allows users to reset their password via email. It works for both user types as long as the user has an email address with an `EmailAddress` record.
...
This prevents user enumeration. A 5-minute cooldown between reset emails is enforced silently.
...
Both verification and reset emails use Jinja2 templates in `app/templates/email/`:
- `verify_email.html` — verification button/link, project name, expiry notice.
- `reset_password.html` — reset button/link, project name, expiry notice, security warning.

Override the directory via `EMAIL_TEMPLATE_DIR`.

### Frontend integration

The email links point to frontend URLs configured in settings:
- `EMAIL_VERIFICATION_URL` (default: `http://localhost:3000/auth/verify-email`) — token appended as `?token=...`
- `PASSWORD_RESET_URL` (default: `http://localhost:3000/auth/reset-password`) — token appended as `?token=...`
...
OAuth is disabled by default. Enable a provider by setting its credentials:
...
4. Backend exchanges the code for an access token, fetches the user's profile (email, name), and either:
   ...
   - Finds an existing user with the same email → auto-links the OAuth identity.
...
The authorization code is single-use — replaying it returns 400.
...
Controlled by `OAUTH_ALLOW_SET_PASSWORD` (default: `true`). Users who already have a password get a 400 — they should use `/change-password` instead.
...
`get_current_user` validates the access token, checks the blacklist, and returns the active `User` instance. It raises 401 if the token is invalid, expired, blacklisted, or the user is inactive.
...
If a refresh token is reused after it has already been rotated:
...
Passwords are validated against `PASSWORD_VALIDATION_REGEX`. The default requires:
- 8 to 100 characters
...
Override the regex in settings:
```bash
# dev.env — example: minimum 12 chars, no special char requirement
PASSWORD_VALIDATION_REGEX='^.{12,100}$'
```

Passwords are hashed with PBKDF2-SHA256 (1,200,000 iterations), compatible with Django's password hashing format. Passwords are automatically rehashed on login if the iteration count has increased since the hash was stored.
...
**Token blacklisting** — Redis-backed. Single-device logout blacklists the specific access token JTI. All-device logout sets a `min_iat` timestamp — any access token issued before that time is rejected. If Redis is unavailable, `FALLBACK_IS_BLACKLISTED` (default: `true`) treats tokens as blacklisted (fail-secure).
...
**CSRF protection** — Opt-in double-submit cookie pattern. Because all authenticated endpoints use Bearer tokens (immune to CSRF), CSRF is not enforced globally. Endpoints authenticating via cookies can opt in with `Depends(csrf_protect)`. See [Security](security.md) for details.
...
Add fields to `AbstractDjangoUser` or `AbstractEmailUser` in `auth/models.py`, then generate a migration:
...
To add relationships from other apps to the User model:
...
See `app/auth/tests/test_views.py` for comprehensive examples.
...
| `OAUTH_LOGIN_REDIRECT_URL` | `"http://localhost:3000/auth/callback"` | Frontend redirect after OAuth callback |
| `OAUTH_ALLOW_SET_PASSWORD` | `true` | Allow OAuth-only users to set a password |
| `OAUTH_CODE_TTL` | `60` | One-time OAuth code TTL in seconds |
| `EMAIL_VERIFICATION` | `"none"` | Verification mode: `"mandatory"`, `"optional"`, `"none"` |
| `EMAIL_VERIFICATION_TOKEN_EXPIRE_SECONDS` | `86400` | Verification token lifetime (default: 24h) |
| `PASSWORD_RESET_TOKEN_EXPIRE_SECONDS` | `3600` | Reset token lifetime (default: 1h) |
| `EMAIL_COOLDOWN_SECONDS` | `300` | Min time between verification/reset emails |
| `EMAIL_VERIFICATION_URL` | `"http://localhost:3000/auth/verify-email"` | Frontend URL for verification links |
| `PASSWORD_RESET_URL` | `"http://localhost:3000/auth/reset-password"` | Frontend URL for reset links |
