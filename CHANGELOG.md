...
## 18-03-26
 ...
 - **Added: `search_fields` exposed in admin schema** (`admin/utils/registry.py`, `admin/schemas.py`, `admin/frontend/`): The schema endpoint now includes `search_fields` per model (list of field names or `null`). The frontend `ListScreen` hides the search bar when `search_fields` is `null`, preventing useless searches on zero-config models with no searchable fields configured.
 - **Security Fix: Admin ordering parameter accepted arbitrary model attributes** (`admin/utils/crud.py`): The `ordering` query parameter was passed to `getattr(model, field_name)` without validation, allowing admins to sort by excluded columns (e.g. `password`) or trigger 500 errors via non-column attributes. Now validated against `entry.fields`.
 - **Fixed: Admin CRUD hardcoded `id` as primary key** (`admin/utils/crud.py`, `admin/views.py`, `admin/frontend/`): `serialize_record`, `list_records`, `get_record`, `update_record`, `delete_record`, `bulk_delete_records`, and `admin_set_password` all assumed `model.id` as the PK. Now uses `entry.pk_field_names` from SQLAlchemy mapper introspection. View routes accept `str` record IDs. Schema exposes `pk_field` so the frontend uses the correct field for row identity, navigation, and selection.
 - **Improved: `BulkDeleteRequest.ids` capped at 500 items** (`admin/schemas.py`): Added `max_length=500` to prevent oversized `DELETE ... WHERE pk IN (...)` queries. Type widened to `list[int | str]` to support non-integer primary keys.
 - **Fixed: Admin session not restored across page reloads** (`admin/frontend/src/api/client.ts`): The 401 handler only attempted token refresh when an access token was already set, creating a catch-22 on page reload (token is in-memory only). Removed the condition so refresh is attempted on any 401 — the HttpOnly refresh cookie handles authentication transparently.
 - **Fixed: Infinite redirect loop on `/admin/`** (`admin/frontend/src/App.tsx`): `getDefaultRoute()` returns `'/'` when schema is not loaded, causing the wildcard route to redirect `/admin/` to itself. `AppRoutes` now shows a spinner during schema load and an error screen with sign-out when schema fails, only rendering the redirect when a valid default route exists.
 - **Fixed: Empty integer/decimal fields coerced to `0` on save** (`admin/frontend/src/components/screens/DetailScreen.tsx`): `parseInt("", 10) || 0` silently converted empty inputs to `0`. Now sends `null` for empty fields, allowing nullable columns to be cleared and non-nullable columns to trigger proper server validation.
 - **Refactored: Removed duplicate frontend type definitions** (`admin/frontend/src/data/types.ts`, `admin/frontend/src/components/screens/DetailScreen.tsx`): `FieldConfig`, `ModelConfig`, `AppConfig`, and `AppsRegistry` in `types.ts` were either unused or duplicates of types in `admin.ts`. Consolidated to use `SchemaField` from `admin.ts`. Only `RecordData` remains in `types.ts`.
 - **Security Fix: Admin password reset did not revoke refresh tokens** (`admin/utils/crud.py`): `admin_set_password` updated the password hash but left existing refresh tokens valid. A compromised account could continue minting access tokens after an admin reset. Now calls `logout_user_all_devices()` after saving, matching the behaviour of the regular `/auth/change-password` endpoint.
 - **Security Fix: Admin user creation without password bypassed `create_user`** (`admin/utils/crud.py`, `admin/views.py`): If the admin submitted a user create request without a password, the code fell through to `model.objects(session).create()` instead of `model.create_user()`, bypassing password hashing. Now explicitly requires a password for user models and returns 400 if missing.
